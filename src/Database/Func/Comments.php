...
class Comments extends DB
{
    ...
    public function profileAnswers($username) {
        $sql = "SELECT answers.*, COALESCE(SUM(answerVotes.vote), 0) as score FROM answers LEFT OUTER JOIN answerVotes ON answerVotes.answerid = answers.id WHERE answers.username = ? GROUP BY answers.id ORDER BY date DESC";
